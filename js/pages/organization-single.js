...
window.addEventListener('load',async()=>{
    ...
    getOneOrganization(orgId).then(res=>{
        ...
        if(res.data.websiteUrl){
            ...
            document.querySelector('#website').parentElement.style.display = "none";
        }
    ...
    document.querySelector('#branches').addEventListener('click',(event)=>{
        event.preventDefault();
        window.localStorage.setItem('samve_directory_branch', orgName);
        window.location.href = '/views/branches.html';
    });
